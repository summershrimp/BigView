	    ...
	    return {
	      ...
	      link: function postLink(scope, element, attrs) {
	        ...
	        function fitWidth(chart, old){
	        	...
	        	scope.option.value = $('.col-lg-9').width()-20;
	        }
	    ...
	    return {
	      ...
	      link: function postLink(scope, element, attrs) {
	        ...
	        scope.scales = [ 
	        	...
	        	{
	        		type : '线性 (数字型)',
	        		value : d3.scale.linear().range(["#5487ba", "#08306b"]),
	        		reset : function(){ this.value.range(["#5487ba", "#08306b"]); },
	        		update : linearUpdate
	        	}
	        ];
    ...
    return {
      ...
      link: function postLink(scope, element, attrs) {
	    ...
	    function over(e,ui){
		    ...
		    html = isValidType(dimension) ? '<i class="fa fa-arrow-circle-down breath-right"></i>放到这里' : '<i class="fa fa-times-circle breath-right"></i>不要放到这里'
		...
		function typeToChinese(type){
			switch(type){
				case "string" : return "文本"; break;
				case "number" : return "数字"; break;
		...
		function message(){
			...
			scope.messageText = hasInvalidType
				? "您只能将 " + scope.types.map(function (d){ return typeToChinese(d.name.toLowerCase()); }).join("、") + " 等类型拖入这里"
				: "将 " + scope.types.map(function (d){ return typeToChinese(d.name.toLowerCase()); }).join(", ") + " 等类型拖入这里";
    ...
    link: function postLink(scope, element, attrs) {
    	...
    	function update(){
    		...
    		if(!scope.data|| !scope.data.length) {
    			d3.select(element[0]).append("span").text("数据貌似不太对，重新确认一下吧。")
    			return;
    		}
    ...
    link: function postLink(scope, element, attrs) {
	    ...
	    client.on( "load", function (client) {
	      client.on( "complete", function (client, args) {
	        ...
	        element.tooltip({ title:'复制成功！'});
	        element.tooltip('show')
	      } );
	    });

	    client.on( 'mouseover', function ( client, args ) {
	    	element.tooltip({title:'复制到剪贴板'});
			  element.tooltip('show');
			});
    ...
    return {
      ...
      template :  '<div class="row">' +
                    '<form class="form-search col-lg-12">' +
                      '<button bs-select class="btn btn-default" placeholder="选择保存类型" ng-model="mode" ng-options="m.label for m in modes">' +
                      'Select <span class="caret"></span>' +
                      '</button>' +
                      '<input class="form-control col-lg-12" placeholder="文件名" type="text" ng-model="filename">' +
                      '<button class="btn btn-success form-control" ng-class="{disabled:!mode}" ng-click="mode.download()">下载</button>' +
                    '</form>' +
                  '</div>',
      ...
      scope.modes = [
    		{ label : '矢量图 (svg)', download : downloadSvg },
    		{ label : '图片 (png)', download : downloadPng },
    		{ label : '数据模型 (json)', download : downloadData }
    	]

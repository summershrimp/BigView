...
angular.module('raw.controllers', [])
  ...
  .controller('RawCtrl', function ($scope, dataService) {
    ...
    $scope.samples = [
      { title : '汽车 (多元)', url : 'data/multivariate.csv' },
      { title : '电影 (离散)', url : 'data/dispersions.csv' },
      { title : '音乐 (流)', url : 'data/flows.csv' },
      { title : '鸡尾酒 (关联)', url : 'data/correlations.csv' }
    ]

    var t="";
    if((t=sessionStorage.getItem("data-filename"))!=null){
      dataService.loadSample(t).then(
        function(data){
          $scope.text = data;
        }, 
        function(error){
    ...
    $scope.delayParse = dataService.debounce($scope.parse, 0, false);
    ...
    $(document).ready(function(){
      ...
      var t="";
      if((t=sessionStorage.getItem("data-filename"))!=null){
        sessionStorage.removeItem("data-filename");
        dataService.loadSample("?page=ajax&action=getFile&filename="+t).then(
          function(data){
            data=data.replace(/^(\n)+/,'');
            $scope.text = data;
          }, 
